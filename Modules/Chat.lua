...
local FormatDiscordHyperlink = function(id)
    return format("|cFF7289DA|Hdiscord:%s|h[%s: %s]|h|r", format("https://discord.gg/%s", id), L["Discord"], id)
end
...
function Chat:CreateChatWindow()
    ...
    self:SetPoint("BOTTOMLEFT", Y.UIParent, 12, 12)
...
function Chat:CreateCopyWindow()
    ...
    Window:SetSize(C["chat-frame-width"] + 6, C["chat-frame-height"] + 6)
    Window:SetPoint("BOTTOM", self, "TOP", 0, 3)
    Window:SetBackdrop(Y.BackdropAndBorder)
    Window:SetBackdropColor(Y:HexToRGB(C["ui-window-bg-color"]))
    ...
    Window.Header:SetBackdrop(Y.BackdropAndBorder)
    ...
    Window.HeaderTexture:SetTexture(A:GetTexture(C["ui-header-texture"]))
    Window.HeaderTexture:SetVertexColor(Y:HexToRGB(C["ui-header-texture-color"]))

    Window.Header.Text = Window.Header:CreateFontString(nil, "OVERLAY")
    Window.Header.Text:SetPoint("LEFT", Window.Header, 5, -1)
    Y:SetFontInfo(Window.Header.Text, C["chat-tab-font"], C["chat-tab-font-size"])
    Window.Header.Text:SetJustifyH("LEFT")
    Window.Header.Text:SetText("|cFF" .. C["chat-tab-font-color"] .. L["Copy text"] .. "|r")
    ...
    Window.CloseButton:SetBackdrop(Y.BackdropAndBorder)
    Window.CloseButton:SetBackdropColor(0, 0, 0, 0)
    Window.CloseButton:SetBackdropBorderColor(0, 0, 0)
    Window.CloseButton:SetScript("OnEnter", function(self) self.Cross:SetVertexColor(Y:HexToRGB("C0392B")) end)
    Window.CloseButton:SetScript("OnLeave", function(self) self.Cross:SetVertexColor(Y:HexToRGB("EEEEEE")) end)
    Window.CloseButton:SetScript("OnMouseUp", function(self)
        self.Texture:SetVertexColor(Y:HexToRGB(C["ui-header-texture-color"]))
    ...
    Window.CloseButton.Texture:SetTexture(A:GetTexture(C["ui-header-texture"]))
    Window.CloseButton.Texture:SetVertexColor(Y:HexToRGB(C["ui-header-texture-color"]))
    ...
    Window.CloseButton.Cross:SetTexture(A:GetTexture("Close"))
    Window.CloseButton.Cross:SetVertexColor(Y:HexToRGB("EEEEEE"))
    ...
    Window.Inner:SetBackdrop(Y.BackdropAndBorder)
    Window.Inner:SetBackdropColor(Y:HexToRGB(C["ui-window-main-color"]))
...
function Chat:StyleChatFrame(frame)
    ...
    frame.CopyHighlight:SetTexture(A:GetTexture("Blank"))
...
function Chat:SetChatTypeInfo()
    _G["CHAT_DISCORD_SEND"] = L["Discord: "]
...
Y:GetModule("GUI"):AddWidgets(L["General"], L["Chat"], function(left, right)
    ...
    left:CreateSwitch("chat-enable-discord-links", C["chat-enable-discord-links"], L["Enable Discord Links"], L["Enable Discord links in the chat frame"])
...
Y:GetModule("GUI"):AddWidgets(L["General"], L["Right"], L["Chat"], function(left, right)
    ...
    right:CreateDropdown("right-window-size", C["right-window-size"], { [L["Single"]] = "SINGLE", [L["Double"]] = "DOUBLE" }, L["Set Window Size"], L["Set the number of windows to be displayed"], ReloadUI):RequiresReload(true)
